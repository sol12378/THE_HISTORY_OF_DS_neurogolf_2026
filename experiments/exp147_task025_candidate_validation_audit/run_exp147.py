...
EXP_DIR = ROOT / "experiments" / "exp147_task025_candidate_validation_audit"
TASK_ID = 25
...
def main() -> None:
    ...
    for row in source_rows:
        ...
        memory, params, score_reason = score_model(utils, raw, TASK_ID, f"exp147_{row['exp']}", EXP_DIR)
        cost = int(memory + params) if memory is not None and params is not None else None
        out_path = EXP_DIR / f"task025_{len(seen):02d}_{row['exp']}.onnx"
    ...
    out_csv = EXP_DIR / "task025_candidate_audit.csv"
    if rows:
    ...
    result: dict[str, Any] = {
        "exp_id": "exp147_task025_candidate_validation_audit",
        "date": "2026-06-10",
        ...
        "decision": "Use the cheapest/fullest public-distinct full_ok candidate for a single-task repair probe if any exist.",
