...
EXP_ID = "exp257_exp256_bypass_bundle_submit_probe"
EXP_DIR = ROOT / "experiments" / EXP_ID
BASE_BUNDLE = ROOT / "experiments" / "exp255_exp254_bypass_bundle_submit_probe" / "submission.zip"
MANIFEST = ROOT / "experiments" / "exp256_current_rank31_80_fullarc_bypass_sweep" / "selected_manifest.csv"
BASE_PUBLIC_LB = 6006.39
...
def score_base_task(utils, raws: dict[int, bytes], proxy_base: dict[int, BaseTask], task_id: int) -> BaseTask:
    ...
    memory, params, reason = score_model(utils, raws[task_id], task_id, f"exp257_base_task{task_id:03d}", EXP_DIR)
...
def main() -> None:
    ...
    result = {
        ...
        "leakage_risk": "low-medium: full-arc gated graph surgery bundle stacked on exp255.",
