...
def main() -> None:
    ...
    result = {
        "exp_id": "exp004_public_blend_relaxed_static",
        ...
        "rule_audit": {
            ...
            "purpose": "officialに許される可能性がある範囲でCompressを許可し、full arc-gen validation済みの6500候補を作る。",
        },
        "next_actions": [
            "local estimate >= 6500 かつ full validation passなら、ユーザー確認後にKaggle submit候補にする。",
            "validation fail taskは次候補へ戻すかstrict rewrite対象へ送る。",
            "public artifact寄せのため、exp006 private-like risk分類を必ず併記する。",
        ],
    }
