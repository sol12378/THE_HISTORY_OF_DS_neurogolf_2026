"""
exp_b036: beicicc_6645 full arc-gen blend

beicicc_6645_artifactはstatic推定で259タスクが安く、合計+699pts相当。
full arc-gen validationを通過した候補についてofficial ORT costで比較し、
genuinely cheaperなものだけを採用して新submission.zipを構築する。
"""
...
EXP_DIR = ROOT / "experiments" / "exp_b036_beicicc_full_arc_blend"
...
SOURCES = {
    "beicicc_6645": ARTIFACT_DIR / "beicicc_6645_artifact" / "submission",
...
def validate(model_bytes, pairs):
    ...
    for inp_g, out_g in pairs:
        if not inp_g or max(len(inp_g), max(len(r) for r in inp_g) if inp_g else 0) > 30:
            n_fail += 1
            continue
        try:
...
def main():
    ...
    print(f"Potential max gain (static): {potential:.4f}")
    ...
    for i, (tid, src_label, alt_data, best_sc, alt_sc) in enumerate(cheaper_tasks):
        ...
        if not pass_all:
            if i < 20:
                print(f"  task{tid:03d}: FAIL ({n_pass}/{n_pass+n_fail}) src={src_label}")
            continue
        ...
        # 公式costでも安い場合のみ採用
        if official_c >= best_sc:
            if i < 30:
                print(f"  task{tid:03d}: official_c={official_c} >= best_sc={best_sc}, SKIP")
            continue
        ...
        for tid in range(1, NUM_TASKS + 1):
            ...
            if tid in updates:
                ...
                try:
                    ...
                    continue
            manifest_rows.append({"task_id": tid, "cost": cost, "points": round(pts(cost), 6)})
    ...
    result = {
        "exp": "exp_b036_beicicc_full_arc_blend",
