...
EXP_DIR = ROOT / "experiments" / "exp168_next4l_candidate_validation_audit"
TARGETS = [48, 35, 12, 17]
...
def audit_task(utils: Any, task_id: int) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    ...
    for row in source_rows:
        ...
        memory, params, score_reason = score_model(utils, raw, task_id, f"exp168_task{task_id:03d}_{row['exp']}", EXP_DIR)
...
def main() -> None:
    ...
    out_csv = EXP_DIR / "next4l_candidate_audit.csv"
    if all_rows:
    ...
    result: dict[str, Any] = {
        "exp_id": "exp168_next4l_candidate_validation_audit",
        "date": "2026-06-10",
        ...
        "decision": "If exp167 identifies a public-zero target, use this audit to pick any distinct full_ok repair candidate before rule mining.",
