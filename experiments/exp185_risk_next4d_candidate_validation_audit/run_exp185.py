...
EXP_DIR = ROOT / "experiments" / "exp185_risk_next4d_candidate_validation_audit"
...
def audit_task(utils: Any, task_id: int) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    ...
    for row in source_rows:
        ...
        memory, params, score_reason = score_model(utils, raw, task_id, f"exp185_task{task_id:03d}_{row['exp']}", EXP_DIR)
...
def main() -> None:
    ...
    out_csv = EXP_DIR / "risk_next4d_candidate_audit.csv"
    if all_rows:
    ...
    result: dict[str, Any] = {
        "exp_id": "exp185_risk_next4d_candidate_validation_audit",
        "date": "2026-06-10",
        ...
        "decision": "Use this audit to pick a distinct full_ok repair candidate for task187 if exp183 identifies task187 as public-zero.",
