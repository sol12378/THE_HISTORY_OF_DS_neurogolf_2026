...
EXP_ID = "exp255_exp254_bypass_bundle_submit_probe"
EXP_DIR = ROOT / "experiments" / EXP_ID
BASE_BUNDLE = ROOT / "experiments" / "exp234_task300_max_color_submit_probe" / "submission.zip"
MANIFEST = ROOT / "experiments" / "exp254_current_top30_fullarc_bypass_sweep" / "selected_manifest.csv"
...
def score_base_task(utils, raws: dict[int, bytes], proxy_base: dict[int, BaseTask], task_id: int) -> BaseTask:
    ...
    memory, params, reason = score_model(utils, raws[task_id], task_id, f"exp255_base_task{task_id:03d}", EXP_DIR)
...
def main() -> None:
    ...
    result = {
        ...
        "failures": failures,
        "local_delta": delta,
        "expected_public_lb_if_calibrated": 6006.32 + delta,
        "zip_sanity": zip_sanity(zip_path),
        "submission_decision": "submit_ready" if selected_rows and not failures else "hold",
        "leakage_risk": "low-medium: full-arc gated graph surgery bundle on current best exp234.",
        "overfitting_risk": "medium-low: validation covers available examples; private robustness still needs LB calibration.",
    }
