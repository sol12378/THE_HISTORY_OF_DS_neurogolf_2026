...
EXP_ID = "exp248_task291_onecell_selector_audit"
...
def color_stats(x: np.ndarray) -> list[dict[str, Any]]:
    ...
    for color, count in sorted(counts.items()):
        ...
        edge_count = int(np.sum((cells[:, 0] == 0) | (cells[:, 0] == x.shape[0] - 1) | (cells[:, 1] == 0) | (cells[:, 1] == x.shape[1] - 1)))
        rows.append(
            {
                "color": int(color),
                "count": int(count),
...
def candidate_rules(stats: list[dict[str, Any]]) -> dict[str, int]:
    ...
    if not stats:
        return rules
    for key in ("count", "bbox_h", "bbox_w", "bbox_area", "r0", "c0", "r1", "c1", "edge_count", "largest_component"):
    ...
    ranked_count = sorted(stats, key=lambda r: (-r["count"], r["color"]))
    for i, row in enumerate(ranked_count[:4]):
...
def main() -> None:
    ...
    for idx, ex in enumerate(examples):
        ...
        ranked_count = sorted(stats, key=lambda r: (-r["count"], r["color"]))
        target_rank = next((i for i, row in enumerate(ranked_count) if row["color"] == target), -1)
        target_rank_hist[target_rank] += 1
        rows.append(
            {
                "idx": idx,
                "target": target,
                "target_count_rank": target_rank,
                "stats": stats,
    ...
    with (EXP_DIR / "onecell_selector_audit.csv").open("w", encoding="utf-8", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=["idx", "target", "target_count_rank", "stats", "rules_hit"])
        writer.writeheader()
    ...
    result = {
        ...
        "target_count_rank_hist": dict(sorted(target_rank_hist.items())),
        "best_rules": ranked[:40],
        "decision": "If a simple selector is full, build 1x1 ONNX cost probe. Otherwise hold task291.",
