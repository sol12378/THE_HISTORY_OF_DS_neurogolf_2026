...
EXP_ID = "exp228_task300_max_color_mask4x3_bgfix_cost_probe"
...
def build_candidate() -> bytes:
    initializers = [
        ...
        numpy_helper.from_array(np.asarray([0, 1, 0, 0], dtype=np.int64), "ch1_starts"),
        numpy_helper.from_array(np.asarray([1, 10, 4, 3], dtype=np.int64), "ch1_ends"),
        ...
        numpy_helper.from_array(np.asarray(1.0, dtype=np.float32), "one_f32"),
        numpy_helper.from_array(np.arange(10, dtype=np.int64).reshape(1, 10), "color_range"),
        numpy_helper.from_array(np.asarray([1, 10, 1, 1], dtype=np.int64), "color_shape"),
        ...
        numpy_helper.from_array(np.asarray([1, 10, 4, 1], dtype=np.int64), "col_tile_repeats"),
        ...
        numpy_helper.from_array(np.asarray(1.0, dtype=np.float32), "pad_one"),
    ]
    nodes = [
        ...
        helper.make_node("Equal", ["color_range", "sel_color"], ["sel_color_bool"]),
        helper.make_node("Cast", ["sel_color_bool"], ["sel_color_float"], to=1),
        helper.make_node("Reshape", ["sel_color_float", "color_shape"], ["sel_color_4d"]),
        helper.make_node("Mul", ["input", "sel_color_4d"], ["selected"]),
        helper.make_node("ReduceSum", ["selected"], ["selected_spatial"], axes=[1], keepdims=1),
        ...
        helper.make_node("GatherElements", ["selected", "row_idx"], ["rows"], axis=2),
        helper.make_node("Add", ["c0", "col_offsets"], ["col_idx_base"]),
        helper.make_node("Tile", ["col_idx_base", "col_tile_repeats"], ["col_idx"]),
        helper.make_node("GatherElements", ["rows", "col_idx"], ["crop4x3_allch"], axis=3),
        helper.make_node("Slice", ["crop4x3_allch", "ch1_starts", "ch1_ends", "axes4"], ["crop4x3_nz"]),
        helper.make_node("ReduceSum", ["crop4x3_nz"], ["occupied"], axes=[1], keepdims=1),
        helper.make_node("Sub", ["one_f32", "occupied"], ["bg_crop"]),
        helper.make_node("Pad", ["bg_crop", "pad_pads", "pad_one"], ["bg_full"], mode="constant"),
        helper.make_node("Pad", ["crop4x3_nz", "pad_pads", "pad_zero"], ["nz_full"], mode="constant"),
...
def main() -> None:
    ...
    candidate = Candidate(
        TASK_ID,
        "max_color_bbox_mask4x3_bgfix",
        base.route,
        build_candidate(),
        "generated",
        "select max nonzero color count, gather 4x3 bbox mask, restore channel0 background",
    )
    ...
    result = {
        ...
        "decision": "improvedならbundle/submission候補。no_cost_gainなら正しいruleでもGatherElements/full-grid intermediatesが重い。",
        "submission_decision": "no_submit: cost probe only",
        "leakage_risk": "low: input-only rule, exp225 validates 267/267.",
