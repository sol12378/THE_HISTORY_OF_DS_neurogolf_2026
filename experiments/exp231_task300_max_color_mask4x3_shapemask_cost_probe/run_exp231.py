...
EXP_ID = "exp231_task300_max_color_mask4x3_shapemask_cost_probe"
...
def build_candidate() -> bytes:
    ...
    nodes = [
        ...
        helper.make_node("ReduceMax", ["occupied"], ["row_any"], axes=[3], keepdims=1),
        helper.make_node("ReduceMax", ["occupied"], ["col_any"], axes=[2], keepdims=1),
        helper.make_node("Mul", ["row_any", "col_any"], ["shape_mask"]),
        helper.make_node("Sub", ["shape_mask", "occupied"], ["bg_crop"]),
...
def main() -> None:
    ...
    candidate = Candidate(
        TASK_ID,
        "max_color_bbox_mask4x3_shapemask",
        base.route,
        build_candidate(),
        "generated",
        "max-color 4x3 mask with dynamic rectangular output shape mask",
    )
    ...
    result = {
        ...
        "decision": "improvedならbundle/submission候補。no_cost_gainなら正しいruleでもこのloweringはcost wall。",
