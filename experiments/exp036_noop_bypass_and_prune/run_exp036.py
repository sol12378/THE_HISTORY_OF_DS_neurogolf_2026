...
EXP_ID = "exp036_noop_bypass_and_prune"
...
def load_base_with_overlay(source_exp: pathlib.Path, raws: dict[int, bytes]) -> dict[int, BaseTask]:
    ...
    overlay_chain = [
        ...
        ROOT / "experiments" / "exp035_greedy_logic_surgery_composition",
    ]
...
def bypass_candidates(task_id: int, raw: bytes, route: str, pass_idx: int) -> Iterable[Candidate]:
    ...
    for idx, node in enumerate(model.graph.node):
        ...
        if node.op_type in BYPASS_OPS and node.input:
            replacements = [(0, node.input[0])]
        elif node.op_type in BINARY_BYPASS_OPS and len(node.input) == 2:
        ...
        for input_idx, input_name in replacements:
            ...
            yield Candidate(
                ...
                "generated",
                f"pass={pass_idx}; bypass {output_name}={node.op_type}(...) with {input_name}",
            )


def sanitize_candidate(task_id: int, raw: bytes, route: str) -> Candidate:
    return Candidate(task_id, "sanitize_prune_only", route, raw, "generated", "official sanitize/prune only")
...
def main() -> None:
    ...
    for task_id in top_task_ids(base, TOP_K):
        ...
        generated += 1
        evaluation, accepted_raw = evaluate_candidate(utils, sanitize_candidate(task_id, current_raw, task.route), current_base, ARC_GEN_SAMPLE, EXP_DIR)
        eval_rows.append(asdict(evaluation))
        if accepted_raw is not None:
            accepted_steps.append(asdict(evaluation))
            current_raw = accepted_raw
            current_base = BaseTask(task_id, int(evaluation.candidate_cost), float(evaluation.candidate_points), EXP_ID, evaluation.template_name, task.route, current_raw)
            task_improved = True

        for pass_idx in range(1, MAX_PASSES_PER_TASK + 1):
            best_eval = None
            best_raw = None
            for candidate in bypass_candidates(task_id, current_raw, task.route, pass_idx):
                generated += 1
        ...
        selected_rows.append(
            {
                ...
                "validation_status": "sample20_pass",
                "status": "improved",
                "reason": "sanitize/no-op bypass composed surgery",
    ...
    result = {
        ...
        "zip_sanity": sanity,
        "leakage_risk": "medium: candidates are validated on sample20 and base contains high-risk lookup artifacts.",
        "overfitting_risk": "medium-high: broad no-op bypass search can overfit validation examples.",
    }
