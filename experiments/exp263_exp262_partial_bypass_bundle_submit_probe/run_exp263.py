...
EXP_ID = "exp263_exp262_partial_bypass_bundle_submit_probe"
EXP_DIR = ROOT / "experiments" / EXP_ID
BASE_BUNDLE = ROOT / "experiments" / "exp261_exp260_bypass_bundle_submit_probe" / "submission.zip"
SOURCE_MANIFEST = ROOT / "experiments" / "exp262_current_rank221_320_fullarc_bypass_sweep" / "selected_manifest.partial.csv"
EXPECTED_BASE_LB = 6007.78
...
def main() -> None:
    ...
    with (EXP_DIR / "fullarc_replay_audit.csv").open("w", encoding="utf-8", newline="") as f:
        fields = ["task_id", "template_name", "full_arc_ok", "passed", "failed", "reason", "sha256"]
        writer = csv.DictWriter(f, fieldnames=fields)
        writer.writeheader()
    ...
    result = {
        ...
        "source_partial_local_delta": 0.5498990065818337,
        ...
        "leakage_risk": "low-medium: replayed exp262 full-arc gated graph surgery on current best bundle.",
