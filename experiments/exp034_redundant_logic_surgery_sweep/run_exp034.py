...
EXP_ID = "exp034_redundant_logic_surgery_sweep"
EXP_DIR = ROOT / "experiments" / EXP_ID
BASE_EXP = ROOT / "experiments" / "exp033_redundant_and_surgery_sweep"
FALLBACK_BASE_EXP = ROOT / "experiments" / "exp023_graph_surgery_exp016"
ARC_GEN_SAMPLE = 20
TOP_K = 160
BINARY_OPS = {"Or", "Greater", "Less", "LessOrEqual", "GreaterOrEqual", "Equal"}
...
def load_base_with_overlay(source_exp: pathlib.Path, raws: dict[int, bytes]) -> dict[int, BaseTask]:
    ...
    result_path = source_exp / "result.json"
    if source_exp == FALLBACK_BASE_EXP or not result_path.exists():
        return base
    result = json.loads(result_path.read_text(encoding="utf-8"))
    for row in result.get("top_selected", []):
        ...
        base[task_id] = BaseTask(
            ...
            source=str(source_exp.relative_to(ROOT)),
...
def surgery_candidates(task_id: int, raw: bytes, route: str) -> Iterable[Candidate]:
    model = onnx.load_model_from_string(raw)
    for idx, node in enumerate(model.graph.node):
        if len(node.output) != 1:
            continue
        output_name = node.output[0]
        replacement_inputs: list[tuple[int, str]] = []
        if node.op_type in BINARY_OPS and len(node.input) == 2:
            replacement_inputs = [(0, node.input[0]), (1, node.input[1])]
        elif node.op_type == "Where" and len(node.input) == 3:
            replacement_inputs = [(1, node.input[1]), (2, node.input[2])]
        else:
            continue
        for input_idx, input_name in replacement_inputs:
            ...
            yield Candidate(
                task_id,
                f"logic_{node.op_type}_node{idx}_to_input{input_idx}",
                route,
...
def main() -> None:
    ...
    for task_id in top_task_ids(base, TOP_K):
        task = base[task_id]
        for candidate in surgery_candidates(task_id, raws[task_id], task.route):
            generated += 1
    ...
    result = {
        ...
        "zip_sanity": sanity,
        "leakage_risk": "medium: graph surgery is accepted only after sample20 validation, but base includes high-risk lookup artifacts.",
        "overfitting_risk": "medium: validation is train/test/arc-gen sample20, not full private-like holdout.",
    }
