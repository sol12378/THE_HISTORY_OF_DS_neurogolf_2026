...
EXP_ID = "exp254_current_top30_fullarc_bypass_sweep"
...
def score_current_base(utils, raws: dict[int, bytes], proxy_base: dict[int, BaseTask], targets: list[int]) -> dict[int, BaseTask]:
    ...
    for task_id in targets:
        ...
        memory, params, reason = score_model(utils, raws[task_id], task_id, f"current_base_task{task_id:03d}", EXP_DIR)
...
def main() -> None:
    ...
    targets = [item.task_id for item in sorted(proxy_base.values(), key=lambda x: (-x.cost, x.task_id))[:TOP_N]]
    ...
    for task_id in targets:
        ...
        if best_eval is not None and best_raw is not None:
            ...
            selected_raws[task_id] = best_raw
    ...
    result = {
        ...
        "local_delta": delta,
        "decision": "If selected_rows non-empty, create bundle on current best and submit micro delta.",
        "submission_decision": "submit_candidate" if selected_rows else "no_submit",
