...
EXP_ID = "exp260_current_rank141_220_fullarc_bypass_sweep"
...
def score_current_base(utils, raws: dict[int, bytes], proxy_base: dict[int, BaseTask], targets: list[int]) -> dict[int, BaseTask]:
    ...
    for task_id in targets:
        ...
        memory, params, reason = score_model(utils, raws[task_id], task_id, f"current_rank141_220_task{task_id:03d}", EXP_DIR)
...
exp259 current bestのcost rank {START_RANK}-{END_RANK} に対して1-pass bypass surgeryをfull-arc gate付きで試す。
