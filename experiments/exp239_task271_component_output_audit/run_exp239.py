...
EXP_ID = "exp239_task271_component_output_audit"
...
def rank0_features(comps: list[dict[str, Any]], comp: dict[str, Any]) -> list[str]:
    hits = []
    for key, reverse in (("size", True), ("area", True), ("r0", False), ("c0", False), ("color", False)):
        ordered = sorted(
            comps,
            key=lambda c: ((-c[key] if reverse else c[key]), c["r0"], c["c0"], c["color"]),
        )
        if ordered and ordered[0] is comp:
            hits.append(f"rank_{key}_{'desc' if reverse else 'asc'}")
    return hits
...
def main() -> None:
    ...
    for idx, ex in enumerate(examples):
        ...
        exactish_matches: list[dict[str, Any]] = []
        shape_match_count = 0
        for ci, comp in enumerate(comps):
            if tuple(y.shape) != (comp["h"], comp["w"]):
                continue
            shape_match_count += 1
            ...
            if rec["crop_exact"] or rec["mask_binary"] or rec["mask_colorized"]:
                ...
                for feat in rank0_features(comps, comp):
                    selector_rank0_hits[feat] += 1
        ...
        counters["component_shape_match"] += int(shape_match_count > 0)
    ...
    with (EXP_DIR / "task271_component_audit.csv").open("w", encoding="utf-8", newline="") as f:
        fieldnames = [
            "idx",
            "output_shape",
            "output_color_count",
    ...
    result = {
        ...
        "selector_rank0_hits": dict(selector_rank0_hits),
        "shape_or_exactish_fail_rows": [r for r in rows if r["shape_match_count"] == 0 or r["exactish_count"] == 0][:20],
        "decision": "If content exactish is near/full and selector is simple, proceed to rule/cost probe. Otherwise pivot.",
