...
EXP_DIR = ROOT / "experiments" / "exp171_next4n_failure_bisection_probe"
BASE_EXP = ROOT / "experiments" / "exp172_task133_158_b035_repair_probe"
BASE_ZIP = BASE_EXP / "submission.zip"
BASE_PUBLIC_LB = 5993.82
TARGETS = [3, 38, 1, 86]
TARGET_POINTS = {
    3: 15.883750551423413,
    38: 17.306974251582112,
    1: 17.047384888349,
    86: 14.83966336368449,
}
...
def make_zero_stub() -> bytes:
    ...
    graph = helper.make_graph(
        ...
        "exp171_zero_fail_stub",
        [helper.make_tensor_value_info("input", TensorProto.FLOAT, [1, 10, 30, 30])],
        [helper.make_tensor_value_info("output", TensorProto.FLOAT, [1, 10, 30, 30])],
        [zero],
    )
    model = helper.make_model(graph, producer_name="exp171_zero_fail_stub", ir_version=10, opset_imports=[helper.make_opsetid("", 10)])
...
def main() -> None:
    ...
    result: dict[str, Any] = {
        "exp_id": "exp171_next4n_failure_bisection_probe",
        "date": "2026-06-10",
        "status": "probe_zip_ready",
        "purpose": "Probe next public-zero bisection group 003/038/001/086 on top of exp172 current best.",
        ...
        "overfitting_risk": "medium: consumes a submission if used and only measures public scoring behavior for a group.",
    }
...
def write_notes(result: dict[str, Any]) -> None:
    lines = [
        ...
        "## 目的",
        "",
        "exp169が全aliveの場合に備え、次候補 `003/038/001/086` のfail-stub probe zipを事前準備する。",
        "",
        "## 結果",
        "",
