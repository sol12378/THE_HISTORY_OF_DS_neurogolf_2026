...
EXP_ID = "exp124_focused_surgery_fourth_pass_limited"
...
CURRENT_LOCAL_ESTIMATE = 6282.950209678613
...
def build_bypass_candidates(base: BaseTask, max_candidates: int = 180) -> list[Candidate]:
    ...
    for idx, node in enumerate(model.graph.node):
        ...
        for input_idx, inp in enumerate(node.input):
            ...
            name = f"fourth_bypass_{node.op_type}_{idx:03d}_in{input_idx}"
            reason = f"fourth pass replace {old} ({node.op_type} node {idx}) with input {input_idx}:{inp}"
...
def main() -> None:
    ...
    result = {
        ...
        "decision": "Fourth focused-surgery pass measures the remaining score-producing tail of the safe post-pass lane.",
        "leakage_risk": "low: graph surgery only and full-validation gated.",
        "overfitting_risk": "low-to-medium: repeated same-task surgery requires LB micro-calibration.",
    }
