...
EXP_ID = "exp111_focused_surgery_second_pass"
...
CURRENT_LOCAL_ESTIMATE = 6282.922189846589
...
def load_seed_tasks() -> dict[int, BaseTask]:
    ...
    with zipfile.ZipFile(PREV_EXPS[-1] / "submission.zip") as zf:
...
def build_bypass_candidates(base: BaseTask, max_candidates: int = 100) -> list[Candidate]:
    ...
    for idx, node in enumerate(model.graph.node):
        ...
        for input_idx, inp in enumerate(node.input):
            ...
            name = f"second_bypass_{node.op_type}_{idx:03d}_in{input_idx}"
            reason = f"second pass replace {old} ({node.op_type} node {idx}) with input {input_idx}:{inp}"
...
def main() -> None:
    ...
    prev_result = json.loads((PREV_EXPS[-1] / "result.json").read_text(encoding="utf-8"))
    targets = sorted(set(prev_result["targets"] + previous_accepted_ids))
    ...
    result = {
        ...
        "decision": "Second pass checks whether focused bypass surgery compounds after exp109/exp110 accepted edits.",
        "leakage_risk": "low: graph surgery only and full-validation gated.",
        "overfitting_risk": "low-to-medium: accepted tasks should be LB-calibrated.",
    }
