...
EXP_DIR = ROOT / "experiments" / "exp198_conv1x1_colormap_miner"
...
def infer_colormap(task_id: int) -> tuple[bool, list[int], str]:
    mapping: dict[int, int] = {}
    for ex_idx, ex in enumerate(examples_for(load_task(task_id), arc_gen_sample=-1)):
...
def build_conv1x1_model(task_id: int, mapping: list[int]) -> bytes:
    weight = np.zeros((10, 10, 1, 1), dtype=np.float32)
    for src, dst in enumerate(mapping):
        weight[dst, src, 0, 0] = 1.0
    bias = np.zeros((10,), dtype=np.float32)
    w = numpy_helper.from_array(weight, "weight")
    b = numpy_helper.from_array(bias, "bias")
    graph = helper.make_graph(
        [helper.make_node("Conv", ["input", "weight", "bias"], ["output"])],
        f"task{task_id:03d}_conv1x1_colormap",
        [helper.make_tensor_value_info("input", TensorProto.FLOAT, [1, 10, 30, 30])],
        [helper.make_tensor_value_info("output", TensorProto.FLOAT, [1, 10, 30, 30])],
        [w, b],
    )
    model = helper.make_model(graph, producer_name="exp198_conv1x1_colormap_miner", ir_version=10, opset_imports=[helper.make_opsetid("", 10)])
...
def main() -> None:
    ...
    for task_id in range(1, 401):
        ok_map, mapping, map_reason = infer_colormap(task_id)
        row: dict[str, Any] = {
            "task_id": task_id,
            "mapping_status": "ok" if ok_map else "skip",
            "mapping_reason": map_reason,
            "mapping": " ".join(map(str, mapping)) if mapping else "",
        ...
        if not ok_map:
            rows.append(row)
            continue
        raw = build_conv1x1_model(task_id, mapping)
        (EXP_DIR / f"task{task_id:03d}_conv1x1_colormap.onnx").write_bytes(raw)
        ...
        if not valid:
            ...
            rows.append(row)
            continue
        memory, params, score_reason = score_model(utils, raw, task_id, "conv1x1_colormap", EXP_DIR)
    ...
    out_csv = EXP_DIR / "conv1x1_colormap_candidates.csv"
    ...
    result = {
        "exp_id": "exp198_conv1x1_colormap_miner",
        "date": "2026-06-10",
        "status": "candidate_scan_complete",
        "purpose": "Instantiate non-injective 1x1 Conv color maps across all tasks as a C-2 one_node_conv_kernel supplier.",
        ...
        "leakage_risk": "low: input-only per-color mapping, no private labels.",
