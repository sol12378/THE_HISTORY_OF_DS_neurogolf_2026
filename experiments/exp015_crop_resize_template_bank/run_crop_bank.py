...
EXP_DIR = ROOT / "experiments" / "exp015_crop_resize_template_bank"
...
def main() -> None:
    ...
    for idx, task_id in enumerate(task_ids, start=1):
        if idx == 1 or idx % 10 == 0:
            print(f"Crop bank {idx}/{len(task_ids)} task{task_id:03d}", flush=True)
        task_examples = examples_for(load_task(task_id), args.arc_gen_sample)
        best = None
        for candidate in crop_candidates(task_id, task_examples, base[task_id].route):
    ...
    result = {
        "exp_id": "exp015_crop_resize_template_bank",
        "date": "2026-06-05",
        ...
        "candidate_status_counts": dict(Counter(row["status"] for row in records)),
        "runtime_seconds": time.time() - started,
    }
    (EXP_DIR / "result.json").write_text(json.dumps(result, ensure_ascii=False, indent=2), encoding="utf-8")
    (EXP_DIR / "notes.md").write_text(
        "# exp015_crop_resize_template_bank notes\n\n"
        "crop/resize routeに対して、identityとcommon fixed crop templateを検証した。\n",
        encoding="utf-8",
    )
